...
private:
	...
	void distStat(std::vector<std::pair<std::string, double>> avbStat) {
...
public:
	...
	void getArtifact() {
		...
		case(1):
			...
			break;
		case(2): {
			const std::vector<std::pair<std::string, double>> avbStat = { {"ATK", 0.2667}, {"HP", 0.2667}, {"DEF", 0.2667}, {"Energy Recharge", 0.1}, {"Elemental Mastery", 0.1} };
			distStat(avbStat);
		}
			   break;
		case(3): {
			const std::vector<std::pair<std::string, double>> avbStat = {
